...
def load(pycsw_config, ckan_url):
    ...
    query = 'api/search/dataset?qjson={"fl":"id,metadata_modified,extras_harvest_object_id,extras_harvest_source_reference", "q":"harvest_object_id:[\\"\\" TO *]", "limit":1000, "start":%s}'
    ...
    while True:
        ...
        for result in results:
            hobj_id = result['extras']['harvest_object_id']
            gathered_records[result['id']] = {
                'metadata_modified': result['metadata_modified'],
                'harvest_object_id': hobj_id,
                'source': result['extras'].get('harvest_source_reference')
            }
...
def get_record(context, repo, ckan_url, ckan_id, ckan_info):
    ...
    url = urlparse.urljoin(ckan_url, '/api/2/rest/harvestobject/%s/xml' % ckan_info['harvest_object_id'])
